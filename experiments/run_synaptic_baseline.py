...
Usage:
------
    python experiments/run_freezing.py --epochs 3 --percentile 0.7 --runs 5
"""
...
from src.models.synaptic_p_factor import SynapticPFactorLIFNetwork, update_synaptic_p_factor_combined
...
def generate_static_mask_and_reset(model, threshold_percentile=0.7):
    ...
    if hasattr(model, 'layer1') and hasattr(model.layer1, 'P'):
        ...
        k = int(p1.numel() * threshold_percentile)
        threshold1 = torch.kthvalue(p1.flatten(), p1.numel() - k + 1).values
        
        # Create mask: 0 for frozen (high P), 1 for plastic (low P)
        mask1 = (p1 < threshold1).float().to(p1.device)
        masks['layer1'] = mask1
        
        # Skip resetting plastic neurons for No-Reset ablation

    if hasattr(model, 'layer2'):
        # Output layer: always freeze Task A outputs (neurons 0-4)
        mask2 = torch.ones_like(model.layer2.linear.weight).to(p1.device)
...
def run_experiment(run_id, epochs, seed, percentile, data_dir=DATA_DIR, is_nmnist=False, alpha_ltp=0.01, alpha_ltd=0.01):
    ...
    model = SynapticPFactorLIFNetwork(input_size=input_dim, hidden_size=HIDDEN_SIZE, output_size=10).to(DEVICE)
    ...
    ckpt_path = os.path.join(ckpt_dir, f"seed_{seed}_epochs_{epochs}_ltp{alpha_ltp}_ltd{alpha_ltd}_synaptic_taskA.pt")
    ...
    if os.path.exists(ckpt_path):
        ...
        for e in range(epochs, 0, -1):
            temp_ckpt = os.path.join(ckpt_dir, f"seed_{seed}_epochs_{e}_ltp{alpha_ltp}_ltd{alpha_ltd}_synaptic_taskA.pt")
    ...
    for epoch in range(start_epoch, epochs):
            ...
            for s, l in pbar:
                ...
                update_synaptic_p_factor_combined(model, preds, l, alpha_ltp=alpha_ltp, alpha_ltd=alpha_ltd)
            ...
            temp_ckpt = os.path.join(ckpt_dir, f"seed_{seed}_epochs_{epoch+1}_ltp{alpha_ltp}_ltd{alpha_ltd}_synaptic_taskA.pt")
            checkpoint = {
    ...
    for epoch in range(epochs):
        ...
        for s, labels in pbar:
            s, labels = s.to(DEVICE), labels.to(DEVICE)
            ...
            update_synaptic_p_factor_combined(model, preds, labels, alpha_ltp, alpha_ltd, p_max=1.0, static_masks=static_masks)
...
if __name__ == "__main__":
    ...
    results_file = f"{output_dir}/freezing_phaseB_synaptic_pfactor_{percentile_int}_ltp{args.alpha_ltp}_ltd{args.alpha_ltd}.json"
