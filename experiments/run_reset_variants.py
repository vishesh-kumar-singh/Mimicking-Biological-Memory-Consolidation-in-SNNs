...
def generate_static_mask_and_reset(model, threshold_percentile=0.7, reset_type="kaiming"):
    ...
    if hasattr(model, 'layer1') and hasattr(model.layer1, 'P'):
        ...
        # Reset weights of plastic neurons based on reset_type
        novice_indices = torch.where(mask1.squeeze() == 1)[0]
        if len(novice_indices) > 0:
            with torch.no_grad():
                if reset_type == "zero":
                    model.layer1.linear.weight[novice_indices] = 0.0
                    if model.layer1.linear.bias is not None:
                        model.layer1.linear.bias[novice_indices] = 0.0
...
def run_experiment(run_id, epochs, seed, percentile, data_dir=DATA_DIR, is_nmnist=False, alpha_ltp=0.01, alpha_ltd=0.01, reset_type="kaiming"):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reset_type", type=str, default="zero", choices=["zero"], help="Reset strategy")
    ...
    while runs_completed < runs_needed:
        if current_seed not in existing_seeds:
            hist = run_experiment(len(histories), args.epochs, current_seed, args.percentile, data_dir=args.data_dir, is_nmnist=args.is_nmnist, alpha_ltp=args.alpha_ltp, alpha_ltd=args.alpha_ltd, reset_type=args.reset_type)
            if hist:
