...
Usage:
------
    python experiments/run_ewc.py --epochs 3 --runs 5 --ewc_lambda 1000.0
"""
...
def run_experiment(run_id, epochs, seed, ewc_lambda, data_dir=DATA_DIR, is_nmnist=False):
    print(f"\n{'='*40}")
...
if __name__ == "__main__":
    ...
    while runs_completed < runs_needed:
        if current_seed not in existing_seeds:
            hist = run_experiment(len(histories), args.epochs, current_seed, args.ewc_lambda, data_dir=args.data_dir, is_nmnist=args.is_nmnist)
            if hist:
