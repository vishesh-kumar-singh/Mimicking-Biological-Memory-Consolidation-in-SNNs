...
Usage:
------
    python experiments/run_si.py --epochs 3 --runs 5 --si_c 1.0 --si_xi 0.1
"""
...
def run_experiment(run_id, epochs, seed, si_c, si_xi, data_dir=DATA_DIR, is_nmnist=False):
    print(f"\n{'='*40}")
...
if __name__ == "__main__":
    ...
    while runs_completed < runs_needed:
        if current_seed not in existing_seeds:
            hist = run_experiment(len(histories), args.epochs, current_seed, args.si_c, args.si_xi, data_dir=args.data_dir, is_nmnist=args.is_nmnist)
            if hist:
