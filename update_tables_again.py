...
def get_ablation_table(epoch, dataset="Split-MNIST"):
    rows = [
        ...
        r'        \midrule',
        r'        \multicolumn{6}{l}{\textit{Novice Reset Strategy}} \\',
...
def replace_table(text, caption_start, replacement):
    # Split on \begin{table*}[t] instead of \begin{table}[H] since we changed it in the previous step
    parts = text.split(r"\begin{table*}[t]")
    for i in range(1, len(parts)):
        if caption_start in parts[i]:
            sub_parts = parts[i].split(r"\end{table*}", 1)
            parts[i] = "\n" + replacement.replace(r"\begin{table*}[t]", "").replace(r"\end{table*}", "") + r"\end{table*}" + sub_parts[1]
    return r"\begin{table*}[t]".join(parts)
...
print("Tables updated: Task B (Task-IL) added, resizebox removed, standard font size restored!")
